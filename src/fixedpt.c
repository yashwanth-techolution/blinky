...
#error Invalid width
	...
	ip = (fixedptud)fixedpt_toint(A);
	do {
		tmp[ndec++] = (char)('0' + (char)(ip % 10));
		ip /= 10;
	} while (ip != 0);
	...
	do {
		...
		str[slen++] = (char)('0' + (char)((fr >> FIXEDPT_BITS) % 10));
		ndec++;
...
fixedpt fixedpt_floor(fixedpt n) {
	return n & ~FIXEDPT_FMASK;
}

fixedpt fixedpt_ceil(fixedpt n) {
    return fixedpt_fracpart(n) ? fixedpt_floor(n) + FIXEDPT_ONE : n;
}
