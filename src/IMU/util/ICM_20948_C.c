...
ICM_20948_Status_e ICM_20948_execute_w(ICM_20948_Device_t *pdev, uint8_t regaddr, uint8_t *pdata, uint32_t len)
{
  ...
  //data_arr[0] = regaddr;
  //memcpy(&data_arr[1], pdata, len);

  ////for(int i=0;i<len;i++)
  ////{
  ////    data_arr[i+1]=pdata[i];
  ////}
  ////printk("Write ->len = %d, reg = %d, data = %d\n", len, regaddr, *pdata);
  //status = i2cSendRegister(data_arr, len+1, ICM_20948_I2C_ADDR_AD1);

  //if(status != 0)
  //{
  //    printf("\n Unable to write data in I2C Peripheral. 'ICM_20948_execute_w'  ");
  //}
  //else
  //{
  //  printf("\n success in write data.");
  //  return ICM_20948_Stat_Ok;
  //}
...
ICM_20948_Status_e ICM_20948_execute_r(ICM_20948_Device_t *pdev, uint8_t regaddr, uint8_t *pdata, uint32_t len)
{
  ...
  ////memcpy(&data_array[1], pdata, len);

  ////status = i2cRead((uint16_t)ICM_20948_I2C_ADDR_AD1, data_array, (uint32_t)len+1);
  //status = i2cReadRegister(ICM_20948_I2C_ADDR_AD1, regaddr_2, 1, pdata, len);
  //if(status != 0)
  //{
  //   printk("\n Unable to read data in I2C Peripheral. ----> 'ICM_20948_execute_r'  ");
  //   //while(1);
  //}
  //else
  //{
  ////printk("Read ->len = %d, reg = %d, data = %d\n", len, regaddr, *pdata);
  //  printf("\n success in read data.");
  //  return ICM_20948_Stat_Ok;
  //}
