...
ICM_20948_Status_e stat = ICM_20948_Stat_NUM;
...
void ICM_20948_debugPrintStatus(ICM_20948_Status_e stat) {
  switch (stat) {
  ...
  case ICM_20948_Stat_NoData:
    printk("Data Underflow");
    break;

  case ICM_20948_Stat_SensorNotSupported:
    printk("Sensor Not Supported");
    break;
    
  case ICM_20948_Stat_DMPNotSupported:
    printk("DMP Firmware Not Supported. Is #define ICM_20948_USE_DMP commented in util/ICM_20948_C.h?");
    break;
    
  case ICM_20948_Stat_DMPVerifyFail:
    printk("DMP Firmware Verification Failed");
    break;
    
  case ICM_20948_Stat_FIFONoDataAvail:
    printk("No FIFO Data Available");
    break;
    
  case ICM_20948_Stat_FIFOIncompleteData:
    printk("DMP data in FIFO was incomplete");
    break;
    
  case ICM_20948_Stat_FIFOMoreDataAvail:
    printk("More FIFO Data Available");
    break;
    
  case ICM_20948_Stat_UnrecognisedDMPHeader:
    printk("Unrecognised DMP Header");
    break;
    
  case ICM_20948_Stat_UnrecognisedDMPHeader2:
    printk("Unrecognised DMP Header2");
    break;
    
  case ICM_20948_Stat_InvalDMPRegister:
    printk("Invalid DMP Register");
    break;
...
const char *ICM_20948_statusString() {
...
ICM_20948_Status_e ICM_20948_setSampleMode(uint8_t sensor_id_bm,
    uint8_t lp_config_cycle_mode) {
  status = ICM_20948_set_sample_mode(&_device,
      (ICM_20948_InternalSensorID_bm)sensor_id_bm,
      (ICM_20948_LP_CONFIG_CYCLE_e)lp_config_cycle_mode);
  k_msleep(1); // Give the ICM20948 time to change the sample mode (see issue #8) 
  return status;
}
...
ICM_20948_Status_e ICM_20948_setFullScale(uint8_t sensor_id_bm, ICM_20948_fss_t fss) {
  status = ICM_20948_set_full_scale(&_device,
      (ICM_20948_InternalSensorID_bm)sensor_id_bm, fss);
  return status;
}
...
ICM_20948_Status_e ICM_20948_setDLPFcfg(uint8_t sensor_id_bm, ICM_20948_dlpcfg_t cfg) {
  status = ICM_20948_set_dlpf_cfg(&_device,
      (ICM_20948_InternalSensorID_bm)sensor_id_bm, cfg);
  return status;
}
...
ICM_20948_Status_e ICM_20948_enableDLPF(uint8_t sensor_id_bm, bool enable) {
  status = ICM_20948_enable_dlpf(&_device,
    (ICM_20948_InternalSensorID_bm)sensor_id_bm, enable);
  return status;
}
...
ICM_20948_Status_e ICM_20948_setSampleRate(uint8_t sensor_id_bm, ICM_20948_smplrt_t smplrt) {
  status = ICM_20948_set_sample_rate(&_device,
    (ICM_20948_InternalSensorID_bm)sensor_id_bm, smplrt);
  return status;
}
...
ICM_20948_Status_e ICM_20948_i2cControllerConfigurePeripheral(uint8_t peripheral,
    uint8_t addr, uint8_t reg, uint8_t len, bool Rw, bool enable, bool data_only,
    bool grp, bool swap, uint8_t dataOut) {
  status = ICM_20948_i2c_controller_configure_peripheral(&_device, peripheral, addr,
      reg, len, Rw, enable, data_only, grp, swap, dataOut);
  return status;
}

//ICM_20948_Status_e ICM_20948::startupDefault(bool minimal)
ICM_20948_Status_e ICM_20948_startupDefault(bool minimal) {
  ICM_20948_Status_e retval = ICM_20948_Stat_Ok;

  retval = ICM_20948_checkID();
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: checkID returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
  }

  retval = ICM_20948_swReset();
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: swReset returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
  ...
  retval = ICM_20948_sleepfunct(false);
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: sleep returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
  ...
  retval = ICM_20948_lowPower(false);
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: lowPower returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
  ...
  if (minimal) { // Return now if minimal is true
    printk("ICM_20948::startupDefault: minimal startup complete!");
    return status;
  }

  retval = ICM_20948_setSampleMode((ICM_20948_Internal_Acc | ICM_20948_Internal_Gyr),
      ICM_20948_Sample_Mode_Continuous); 
      // options: ICM_20948_Sample_Mode_Continuous or ICM_20948_Sample_Mode_Cycled
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: setSampleMode returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
  ...
  retval = ICM_20948_setFullScale((ICM_20948_Internal_Acc | ICM_20948_Internal_Gyr), FSS);
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: setFullScale returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
  ...
  retval = ICM_20948_setDLPFcfg((ICM_20948_Internal_Acc | ICM_20948_Internal_Gyr), dlpcfg);
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: setDLPFcfg returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
  ...
  retval = ICM_20948_enableDLPF(ICM_20948_Internal_Acc, false);
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: enableDLPF (Acc) returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
  ...
  retval = ICM_20948_enableDLPF(ICM_20948_Internal_Gyr, false);
  if (retval != ICM_20948_Stat_Ok) {
    printk("ICM_20948::startupDefault: enableDLPF (Gyr) returned: ");
    ICM_20948_debugPrintStatus(retval);
    printk("\n");
    status = retval;
    return status;
...
ICM_20948_Status_e ICM_20948_enableDMPSensor(enum inv_icm20948_sensor sensor, bool enable) {
  if (_device._dmp_firmware_available == true) { // Should we attempt to enable the sensor?
    ...
    printk("ICM_20948::enableDMPSensor:  _enabled_Android_0: ");
    printk((int)_device._enabled_Android_0);
    printk("  _enabled_Android_1: ");
    printk((int)_device._enabled_Android_1);
    printk("  _dataOutCtl1: ");
    printk((int)_device._dataOutCtl1);
    printk("  _dataOutCtl2: ");
    printk((int)_device._dataOutCtl2);
    printk("  _dataRdyStatus: ");
    printk((int)_device._dataRdyStatus);
    printk("\n");
    return status;
  }
  return ICM_20948_Stat_DMPNotSupported;
...
ICM_20948_Status_e ICM_20948_writeDMPmems(unsigned short reg, unsigned int length,
...
ICM_20948_Status_e ICM_20948_setGyroSF(unsigned char div, int gyro_level) {
  if (_device._dmp_firmware_available == true) { // Should we attempt to set the Gyro SF?
    ...
    printk("ICM_20948::setGyroSF:  pll: ");
    printk((int)_device._gyroSFpll);
    printk("  Gyro SF is: ");
    //printk((int)_device._gyroSF);
    printk("\n");
    return status;
  }
  // TODO: please check nordic getting reseteded.
  // NOTE: same.
  return ICM_20948_Stat_DMPNotSupported;
}
...
ICM_20948_Status_e ICM_20948_initializeDMP(void) {
  unsigned short address = DMP_START_ADDRESS;

  // First, let's check if the DMP is available
  if (_device._dmp_firmware_available != true) {
    printk("ICM_20948::startupDMP: DMP is not available. Please check that you"
    "have uncommented line 29 (#define ICM_20948_USE_DMP) in ICM_20948_C.h...");
    return ICM_20948_Stat_DMPNotSupported;
  }
  ...
  result = ICM_20948_i2cControllerConfigurePeripheral(0, MAG_AK09916_I2C_ADDR, AK09916_REG_RSV2, 10, true, true, false, true, true, 0); if (result > worstResult) worstResult = result;
  //
  ...
  // false: clear the I2C_SLV0_CTRL I2C_SLV 0_REG_DIS (we want to write the register value)
  // false: clear the I2C_SLV0_CTRL I2C_SLV0_GRP bit
  // false: clear the I2C_SLV0_CTRL I2C_SLV0_BYTE_SW bit
  // AK09916_mode_single: tell I2C_SLV1 to write the Single Measurement command each sample
  result = ICM_20948_i2cControllerConfigurePeripheral(1, MAG_AK09916_I2C_ADDR, AK09916_REG_CNTL2, 1, false, true, false, false, false, AK09916_mode_single); if (result > worstResult) worstResult = result;
  ...
  result = ICM_20948_setBank(3); if (result > worstResult) worstResult = result; // Select Bank 3
  uint8_t mstODRconfig = 0x04; // Set the ODR configuration to 1100/2^4 = 68.75Hz
  result = ICM_20948_write(AGB3_REG_I2C_MST_ODR_CONFIG, &mstODRconfig, 1); if (result > worstResult) worstResult = result; // Write one byte to the I2C_MST_ODR_CONFIG register  

  // Configure clock source through PWR_MGMT_1
  // ICM_20948_Clock_Auto selects the best available clock source – PLL if ready, else use the Internal oscillator
  result = ICM_20948_setClockSource(ICM_20948_Clock_Auto); if (result > worstResult) worstResult = result; // This is shorthand: success will be set to false if setClockSource fails

  // Enable accel and gyro sensors through PWR_MGMT_2
  // Enable Accelerometer (all axes) and Gyroscope (all axes) by writing zero to PWR_MGMT_2
  result = ICM_20948_setBank(0); if (result > worstResult) worstResult = result;                               // Select Bank 0
  uint8_t pwrMgmt2 = 0x40;                                                          // Set the reserved bit 6 (pressure sensor disable?)
  result = ICM_20948_write(AGB0_REG_PWR_MGMT_2, &pwrMgmt2, 1); if (result > worstResult) worstResult = result; // Write one byte to the PWR_MGMT_2 register

  // Place _only_ I2C_Master in Low Power Mode (cycled) via LP_CONFIG
  // The InvenSense Nucleo example initially puts the accel and gyro into low power mode too, but then later updates LP_CONFIG so only the I2C_Master is in Low Power Mode
  result = ICM_20948_setSampleMode(ICM_20948_Internal_Mst, ICM_20948_Sample_Mode_Cycled); if (result > worstResult) worstResult = result;
 
  // Disable the FIFO
  result = ICM_20948_enableFIFO(false); if (result > worstResult) worstResult = result;

  // Disable the DMP
  result = ICM_20948_enableDMP(false); if (result > worstResult) worstResult = result;
  ...
  myFSS.g = dps2000;     // (ICM_20948_GYRO_CONFIG_1_FS_SEL_e)
                         ...
                         // dps1000
                         // dps2000
  result = ICM_20948_setFullScale((ICM_20948_Internal_Acc | ICM_20948_Internal_Gyr), myFSS); if (result > worstResult) worstResult = result;

  // The InvenSense Nucleo code also enables the gyro DLPF (but leaves GYRO_DLPFCFG set to zero = 196.6Hz (3dB))
  // We found this by going through the SPI data generated by ZaneL's Teensy-ICM-20948 library byte by byte...
  // The gyro DLPF is enabled by default (GYRO_CONFIG_1 = 0x01) so the following line should have no effect, but we'll include it anyway
  result = ICM_20948_enableDLPF(ICM_20948_Internal_Gyr, true); if (result > worstResult) worstResult = result;
  ...
  result = ICM_20948_setBank(0); if (result > worstResult) worstResult = result; // Select Bank 0
  uint8_t zero = 0;
  result = ICM_20948_write(AGB0_REG_FIFO_EN_1, &zero, 1); if (result > worstResult) worstResult = result;
  // Stop the accelerometer, gyro and temperature data from being written to the FIFO by writing zero to FIFO_EN_2
  result = ICM_20948_write(AGB0_REG_FIFO_EN_2, &zero, 1); if (result > worstResult) worstResult = result;
 

  // Turn off data ready interrupt through INT_ENABLE_1
  result = ICM_20948_intEnableRawDataReady(false); if (result > worstResult) worstResult = result;

  // Reset FIFO through FIFO_RST
  result = ICM_20948_resetFIFO(); if (result > worstResult) worstResult = result;
  ...
  //mySmplrt.g = 8; // 112Hz
  //mySmplrt.a = 8; // 112Hz
  result = ICM_20948_setSampleRate((ICM_20948_Internal_Acc | ICM_20948_Internal_Gyr), mySmplrt); if (result > worstResult) worstResult = result;

  // Setup DMP start address through PRGM_STRT_ADDRH/PRGM_STRT_ADDRL
  result = ICM_20948_setDMPstartAddress(address); if (result > worstResult) worstResult = result; // Defaults to DMP_START_ADDRESS
 
  // Now load the DMP firmware
  result = ICM_20948_loadDMPFirmware(); if (result > worstResult) worstResult = result;
  ...
  result = ICM_20948_setBank(0); if (result > worstResult) worstResult = result; // Select Bank 0
  uint8_t fix = 0x48;
  result = ICM_20948_write(AGB0_REG_HW_FIX_DISABLE, &fix, 1); if (result > worstResult) worstResult = result;

  // Set the Single FIFO Priority Select register to 0xE4
  result = ICM_20948_setBank(0); if (result > worstResult) worstResult = result; // Select Bank 0
  uint8_t fifoPrio = 0xE4;
  result = ICM_20948_write(AGB0_REG_SINGLE_FIFO_PRIORITY_SEL, &fifoPrio, 1); if (result > worstResult) worstResult = result;

  // Configure Accel scaling to DMP
  // The DMP scales accel raw data internally to align 1g as 2^25
  // In order to align internal accel raw data 2^25 = 1g write 0x04000000 when FSR is 4g
  const unsigned char accScale[4] = {0x04, 0x00, 0x00, 0x00};
  result = ICM_20948_writeDMPmems(ACC_SCALE, 4, &accScale[0]); if (result > worstResult) worstResult = result; // Write accScale to ACC_SCALE DMP register
  // In order to output hardware unit data as configured FSR write 0x00040000 when FSR is 4g
  const unsigned char accScale2[4] = {0x00, 0x04, 0x00, 0x00};
  result = ICM_20948_writeDMPmems(ACC_SCALE2, 4, &accScale2[0]); if (result > worstResult) worstResult = result; // Write accScale2 to ACC_SCALE2 DMP register
  ...
  result = ICM_20948_writeDMPmems(CPASS_MTX_00, 4, &mountMultiplierPlus[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(CPASS_MTX_01, 4, &mountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(CPASS_MTX_02, 4, &mountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(CPASS_MTX_10, 4, &mountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(CPASS_MTX_11, 4, &mountMultiplierMinus[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(CPASS_MTX_12, 4, &mountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(CPASS_MTX_20, 4, &mountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(CPASS_MTX_21, 4, &mountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(CPASS_MTX_22, 4, &mountMultiplierMinus[0]); if (result > worstResult) worstResult = result;

  // Configure the B2S Mounting Matrix
  const unsigned char b2sMountMultiplierZero[4] = {0x00, 0x00, 0x00, 0x00};
  const unsigned char b2sMountMultiplierPlus[4] = {0x40, 0x00, 0x00, 0x00}; // Value taken from InvenSense Nucleo example
  result = ICM_20948_writeDMPmems(B2S_MTX_00, 4, &b2sMountMultiplierPlus[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(B2S_MTX_01, 4, &b2sMountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(B2S_MTX_02, 4, &b2sMountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(B2S_MTX_10, 4, &b2sMountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(B2S_MTX_11, 4, &b2sMountMultiplierPlus[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(B2S_MTX_12, 4, &b2sMountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(B2S_MTX_20, 4, &b2sMountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(B2S_MTX_21, 4, &b2sMountMultiplierZero[0]); if (result > worstResult) worstResult = result;
  result = ICM_20948_writeDMPmems(B2S_MTX_22, 4, &b2sMountMultiplierPlus[0]); if (result > worstResult) worstResult = result;
  ...
  result = ICM_20948_setGyroSF(19, 3); if (result > worstResult) worstResult = result; // 19 = 55Hz (see above), 3 = 2000dps (see above)
  ...
  result = ICM_20948_writeDMPmems(GYRO_FULLSCALE, 4, &gyroFullScale[0]); if (result > worstResult) worstResult = result;

  // Configure the Accel Only Gain: 15252014 (225Hz) 30504029 (112Hz) 61117001 (56Hz)
  const unsigned char accelOnlyGain[4] = {0x03, 0xA4, 0x92, 0x49}; // 56Hz
  //const unsigned char accelOnlyGain[4] = {0x00, 0xE8, 0xBA, 0x2E}; // 225Hz
  //const unsigned char accelOnlyGain[4] = {0x01, 0xD1, 0x74, 0x5D}; // 112Hz
  result = ICM_20948_writeDMPmems(ACCEL_ONLY_GAIN, 4, &accelOnlyGain[0]); if (result > worstResult) worstResult = result;

  // Configure the Accel Alpha Var: 1026019965 (225Hz) 977872018 (112Hz) 882002213 (56Hz)
  const unsigned char accelAlphaVar[4] = {0x34, 0x92, 0x49, 0x25}; // 56Hz
  //const unsigned char accelAlphaVar[4] = {0x3D, 0x27, 0xD2, 0x7D}; // 225Hz
  //const unsigned char accelAlphaVar[4] = {0x3A, 0x49, 0x24, 0x92}; // 112Hz
  result = ICM_20948_writeDMPmems(ACCEL_ALPHA_VAR, 4, &accelAlphaVar[0]); if (result > worstResult) worstResult = result;

  // Configure the Accel A Var: 47721859 (225Hz) 95869806 (112Hz) 191739611 (56Hz)
  const unsigned char accelAVar[4] = {0x0B, 0x6D, 0xB6, 0xDB}; // 56Hz
  //const unsigned char accelAVar[4] = {0x02, 0xD8, 0x2D, 0x83}; // 225Hz
  //const unsigned char accelAVar[4] = {0x05, 0xB6, 0xDB, 0x6E}; // 112Hz
  result = ICM_20948_writeDMPmems(ACCEL_A_VAR, 4, &accelAVar[0]); if (result > worstResult) worstResult = result;

  // Configure the Accel Cal Rate
  const unsigned char accelCalRate[4] = {0x00, 0x00}; // Value taken from InvenSense Nucleo example
  result = ICM_20948_writeDMPmems(ACCEL_CAL_RATE, 2, &accelCalRate[0]); if (result > worstResult) worstResult = result;

  // Configure the Compass Time Buffer. The I2C Master ODR Configuration (see above) sets the magnetometer read rate to 68.75Hz.
  // Let's set the Compass Time Buffer to 69 (Hz).
  const unsigned char compassRate[2] = {0x00, 0x45}; // 69Hz
  result = ICM_20948_writeDMPmems(CPASS_TIME_BUFFER, 2, &compassRate[0]); if (result > worstResult) worstResult = result;
...
ICM_20948_Status_e ICM_20948_I2C_begin() {
  ...
  status = ICM_20948_startupDefault(_device._dmp_firmware_available);
  
  if (status != ICM_20948_Stat_Ok) {
    printk("ICM_20948_I2C::begin: startupDefault returned: ");
    ICM_20948_debugPrintStatus(status);
    printk("\n");
  }
  return status;
...
ICM_20948_Status_e ICM_20948_write_I2C(uint8_t reg, uint8_t *data, uint32_t len) {
  uint8_t tempdata[len+1];
  int ret = 0;
  tempdata[0] = reg;
  memcpy(&tempdata[1], data, len);

  //printk("Write ->len = %d, reg = %d, data = %d\n", len, reg, *data);

  ret = i2cSendRegister(tempdata, len+1, ICM_20948_DEFAULT_ADDRESS);
   //k_msleep(5);
  if(ret != 0) {
    printk("Error (%d) Sending IMU Data\n", ret);
  }
  //printk("ICM_20948_write_I2C is working all fine\n");
...
ICM_20948_Status_e ICM_20948_read_I2C(uint8_t reg, uint8_t *buff, uint32_t len) {
  int num_received = 0;
  num_received = i2cReadRegister((uint16_t) ICM_20948_DEFAULT_ADDRESS, &reg, 1, buff, len);
