...
//    static void stopServer(const uint8_t sock);
    ...
    //static bool ServerSpiDrv_startClient(const uint32_t ipAddress, const uint16_t port, const uint8_t sock, const uint8_t protMode=TCP_MODE);
    bool ServerSpiDrv_startClient(const uint32_t ipAddress, const uint16_t port, const uint8_t sock, const uint8_t protMode);

    void ServerSpiDrv_stopClient(const uint8_t sock);
                                                                                  
//    static uint8_t getServerState(const uint8_t sock);

    uint8_t ServerSpiDrv_getClientState(const uint8_t sock);

    //static bool ServerSpiDrv_getData(const uint8_t sock, int16_t *data, uint8_t peek = 0);
    bool ServerSpiDrv_getData(const uint8_t sock, int16_t *data, uint8_t peek);

    bool ServerSpiDrv_getDataBuf(const uint8_t sock, uint8_t *data, uint16_t *len);
